...
static volatile int has_commutated; // 驱动状态 (has_commutated == true)，滑行状态 (has_commutated == false)
...
static void pll_run(float phase, float dt, volatile float *phase_var, volatile float *speed_var)
{
    ...
    *phase_var += (*speed_var + conf->foc_pll_kp * delta_theta) * dt; // P+I项
    utils_norm_angle_rad((float *)phase_var);
    *speed_var += conf->foc_pll_ki * delta_theta * dt; // I项
}
 ...
 * The duty cycle to use.
 */
void mcpwm_set_duty_noramp(float dutyCycle) // 设置占空比（带/不带斜率限制）
{
    control_mode = CONTROL_MODE_DUTY;
 ...
 * The current to use.
 */
void mcpwm_set_current(float current) // 设置电流目标
{
    ...
    utils_truncate_number(&current, -conf->l_current_max * conf->l_current_max_scale, conf->l_current_max * conf->l_current_max_scale);
...
{
    float value = filter_run_fir_iteration((float *)kv_fir_samples, (float *)kv_fir_coeffs, KV_FIR_TAPS_BITS, kv_fir_index);
...
static void set_duty_cycle_ll(float dutyCycle)
{
    ...
    if (dutyCycle < conf->l_min_duty) {
        ...
        if (control_mode == CONTROL_MODE_CURRENT || control_mode == CONTROL_MODE_CURRENT_BRAKE || control_mode == CONTROL_MODE_SPEED) {
#else
...
static void set_duty_cycle_hw(float dutyCycle)
{
    ...
    if (conf->motor_type == MOTOR_TYPE_BLDC && conf->pwm_mode == PWM_MODE_BIPOLAR && !IS_DETECTING()) {
        timer_tmp.duty = (uint16_t)(((float)timer_tmp.top / 2.0) * dutyCycle + ((float)timer_tmp.top / 2.0));
    } else {
...
static THD_FUNCTION(rpm_thread, arg)
{
    ...
    for (;;) {
        ...
        rpm_dep.cycle_int_limit_running = rpm_dep.cycle_int_limit + (float)ADC_Value[ADC_IND_VIN_SENS] * conf->sl_bemf_coupling_k /
                                                                        (rpm_abs > conf->sl_min_erpm ? rpm_abs : conf->sl_min_erpm);
        rpm_dep.cycle_int_limit_running = utils_map(rpm_abs, 0, conf->sl_cycle_int_rpm_br, rpm_dep.cycle_int_limit_running,
                                                    rpm_dep.cycle_int_limit_running * conf->sl_phase_advance_at_br);
        rpm_dep.cycle_int_limit_max =
            rpm_dep.cycle_int_limit + (float)ADC_Value[ADC_IND_VIN_SENS] * conf->sl_bemf_coupling_k / conf->sl_min_erpm_cycle_int_limit;
...
static THD_FUNCTION(timer_thread, arg)
{
    ...
    for (;;) {
        ...
        if (state == MC_STATE_OFF) {
            ...
            amp = filter_run_fir_iteration((float *)amp_fir_samples, (float *)amp_fir_coeffs, AMP_FIR_TAPS_BITS, amp_fir_index);
        ...
        if (cnt_tmp >= 10) {
            cnt_tmp = 0;
            if (state == MC_STATE_RUNNING) {
                filter_add_sample((float *)kv_fir_samples, mcpwm_get_kv(), KV_FIR_TAPS_BITS, (uint32_t *)&kv_fir_index);
...
void mcpwm_adc_inj_int_handler(void)
{
    ...
    if (conf->motor_type == MOTOR_TYPE_DC) {
        ...
        if (detect_now == 4) {
            ...
            if (detect_step > 0) {
                mcpwm_detect_currents_diff[detect_step] = mcpwm_detect_currents[detect_step - 1] - mcpwm_detect_currents[detect_step];
            } else {
                mcpwm_detect_currents_diff[detect_step] = mcpwm_detect_currents[5] - mcpwm_detect_currents[detect_step];
            }
    ...
    filter_add_sample((float *)current_fir_samples, last_current_sample, CURR_FIR_TAPS_BITS, (uint32_t *)&current_fir_index);
    last_current_sample_filtered =
        filter_run_fir_iteration((float *)current_fir_samples, (float *)current_fir_coeffs, CURR_FIR_TAPS_BITS, current_fir_index);
 ...
 */
void mcpwm_adc_int_handler(void *p, uint32_t flags) // ADC 中断处理 — 核心运行时：采样电流、换相、PID
{
    (void)p;
    (void)flags;
    ...
    if (conf->motor_type == MOTOR_TYPE_BLDC) {
        ...
        filter_add_sample((float *)amp_fir_samples, amp, AMP_FIR_TAPS_BITS, (uint32_t *)&amp_fir_index);
        ...
        if (sensorless_now) {
            ...
            if ((state == MC_STATE_RUNNING && pwm_cycles >= 2) || state == MC_STATE_OFF) {
                ...
                if (conf->comm_mode == COMM_MODE_INTEGRATE) { // 积分模式
                    float limit;
                    if (has_commutated) {
                    ...
                    if (cycle_integrator >= (rpm_dep.cycle_int_limit_max * (0.0005 * VDIV_CORR)) || cycle_integrator >= limit) {
                        commutate(1);
                        cycle_integrator = 0.0;
                        cycle_sum = 0.0;
                    }
                } else if (conf->comm_mode == COMM_MODE_DELAY) { // 延迟模式
                    if (v_diff > 0) { // 确认已经过了过零点
                        cycle_sum += conf->m_bldc_f_sw_max / switching_frequency_now; // 累加时间

                        // 核心：动态计算延迟时间阈值(低速时,标准30°延迟;高速时,(comm_time_sum / 2.0) * sl_phase_advance_at_br)
                        if (cycle_sum >= utils_map(fabsf(rpm_now), 0, conf->sl_cycle_int_rpm_br, rpm_dep.comm_time_sum / 2.0,
        ...
        if (state == MC_STATE_RUNNING && !has_commutated) {
            set_next_comm_step(comm_step);
            commutate(0); // 立即将配置应用到硬件
        }
    }

    const float current_nofilter = mcpwm_get_tot_current(); // 相电流
    const float current_in_nofilter = current_nofilter * fabsf(dutycycle_now); // 母线输入电流
    ...
    if (state == MC_STATE_RUNNING && has_commutated) {
        ...
        if (control_mode == CONTROL_MODE_CURRENT || control_mode == CONTROL_MODE_POS) {
            ...
            if (fabsf(dutycycle_now_tmp) < start_boost) { // 启动的Boost（克服静摩擦）
                utils_step_towards(&dutycycle_now_tmp, current_set > 0.0 ? start_boost : -start_boost,
                                   ramp_step); // 强制以较快的斜坡ramp_step将其拉升到 Boost 值,瞬间突破静摩擦
            } else {
        ...
        } else if (control_mode == CONTROL_MODE_CURRENT_BRAKE) { // 电流刹车模式
            // Compute error
            ...
            // Lower truncation
            if (fabsf(dutycycle_now_tmp) < conf->l_min_duty) {
                if (fabsf(rpm_now) < conf->l_max_erpm_fbrake_cc) { // 低速直接松开刹车
            ...
            // // 软退让：按照超限程度，平滑地把占空比往dutycycle_set拉
            utils_step_towards((float *)&dutycycle_now_tmp, dutycycle_set, ramp_step);
        }

        static int limit_delay = 0;

        // 检查四个极限：相电流最大/最小，母线电流最大/最小.当遇到大负载时，电流会温柔地贴在限流值（如 100A）
        // Apply limits in priority order
        if (current_nofilter > conf->lo_current_max) { // 软退让：按照超限程度，平滑地把占空比往 0 拉
        ...
        if (fabsf(dutycycle_now) < conf->l_min_duty) { // 防低速停机（维持最小占空比）
        ...
        if (conf->motor_type != MOTOR_TYPE_DC) { // 防反转炸机（绝对红线）--逆风启动
            const float rpm = mcpwm_get_rpm();
    ...
    if (encoder_is_configured()) {
        ...
        run_pid_control_pos(1.0 / switching_frequency_now, pos); // 位置环 PID
        pll_run(-DEG2RAD_f(pos), 1.0 / switching_frequency_now, &m_pll_phase, &m_pll_speed); // PLL 测速
    }
...
{
    if (mc_interface_try_input()) { // 系统检查
        return;
    }
...
// 利用电机的“磁饱和效应”或“凸极效应”，通过向6个方向注入测试电压并测量电流响应，反推出转子永磁体 N 极所指的精确电角度。
float mcpwm_get_detect_pos(void)
{
    ...
    // 差分提取，抵消掉与位置无关的直流偏置，提取出纯粹的、由磁路不对称引起的幅值差异
    float v0 = v[0] + v[3];
    float v1 = v[1] + v[4];
    float v2 = v[2] + v[5];
    ...
    float ph[1];
    ph[0] = RAD2DEG_f(asinf(v0)); // 计算 A 相分量对应的角度

    float res = ph[0];
    if (v1 < v2) { // 根据 B 相和 C 相的大小判断象限
        res = 180 - ph[0];
    }

    utils_norm_angle(&res); // 归一化到 0-360 度

    return res;
}

// 在过去 N 次换相中，每次触发换相时BEMF积分面积的平均值
 ...
 * -3: Encoder enabled
 */
int mcpwm_get_hall_detect_result(int8_t *table) // 霍尔传感器自动检测
{
    if (conf->m_sensor_port_mode != SENSOR_PORT_MODE_HALL) {
        return -3; // 返回 -3：编码器已启用
    }

    // 霍尔状态为 i 时，系统处于换相步骤 j 的累计次数
    for (int i = 0; i < 8; i++) { // 霍尔状态为 i
        int samples = 0;
        int res = -1;
        for (int j = 1; j < 7; j++) { // 遍历 6 个换相步骤 (1 到 6)
            if (hall_detect_table[i][j] > samples) {
                samples = hall_detect_table[i][j];
                if (samples > 15) { // 核心阈值：至少被统计到 15 次
                    res = j;
                }
            }
            table[i] = res;
    ...
    int tot_nums = 0;
    for (int i = 0; i < 8; i++) {
        if (table[i] == -1) { // 统计无效状态的数量
            invalid_samp_num++;
        } else {
            if (!nums[table[i]]) { // 利用数组下标进行去重
                nums[table[i]] = 1;
                tot_nums++; // 统计不重复的有效映射数量
            }
        }
    }

    // 8 种状态中，必须且只能有 2 种是无效的（即映射为 -1）
...
static void update_sensor_mode(void)
{
    if (conf->sensor_mode == SENSOR_MODE_SENSORLESS || (conf->sensor_mode == SENSOR_MODE_HYBRID && fabsf(mcpwm_get_rpm()) > conf->hall_sl_erpm)) {
        sensorless_now = true;
    } else {
 ...
 */
static void update_timer_attempt(void) // 预装载->更新事件
{
    utils_sys_lock_cnt();
