 ...
 */
void virtual_motor_init(volatile mc_configuration *conf) // 初始化，注册终端命令
{
    ...
    terminal_register_command_callback("connect_virtual_motor", "connects virtual motor", "[ml][J][Vbus]", terminal_cmd_connect_virtual_motor);

    terminal_register_command_callback("disconnect_virtual_motor", "disconnect virtual motor", 0, terminal_cmd_disconnect_virtual_motor);
}
 ...
 */
void virtual_motor_int_handler(float v_alpha, float v_beta) // 主入口 — 接收 v_alpha/v_beta，运行模型，回写 ADC
{
...
static void connect_virtual_motor(float ml, float J, float Vbus)
{
    if (virtual_motor.connected == false) {
        ...
        mcpwm_foc_get_current_offsets(&m_curr0_offset_backup, &m_curr1_offset_backup, &m_curr2_offset_backup, false);
        ...
        if (tempVoltage != 0.0) {
            ADC_Value[ADC_IND_VOUT_GATE_DRV] =
                ((HW_GATE_DRIVER_SUPPLY_MAX_VOLTAGE + HW_GATE_DRIVER_SUPPLY_MIN_VOLTAGE) / 2.0) / GET_GATE_DRIVER_SUPPLY_VOLTAGE();
        }
#endif
...
static void disconnect_virtual_motor(void)
{
    ...
    if (virtual_motor.connected) {
        ...
        mcpwm_foc_set_current_offsets(m_curr0_offset_backup, m_curr1_offset_backup, m_curr2_offset_backup);
 ...
 */
static inline void run_virtual_motor_electrical(float v_alpha, float v_beta) // 电气模型 — dq 轴电流方程积分
{
    ...
    // d axis current
    virtual_motor.id_int += ((virtual_motor.vd + virtual_motor.we * virtual_motor.pole_pairs * virtual_motor.lq * virtual_motor.iq -
    ...
    // q axis current
    virtual_motor.iq +=
        (virtual_motor.vq - virtual_motor.we * virtual_motor.pole_pairs * (virtual_motor.ld * virtual_motor.id + m_conf->foc_motor_flux_linkage) -
 ...
 */
static inline void run_virtual_motor_mechanics(float ml) // 机械模型 — 转矩与运动方程
{
    virtual_motor.me =
        virtual_motor.km * (m_conf->foc_motor_flux_linkage + (virtual_motor.ld - virtual_motor.lq) * virtual_motor.id) * virtual_motor.iq;
    // omega
 ...
 */
static inline void run_virtual_motor_park_clark_inverse(void) // Park-1 + Clark-1 变换 + 回写 ADC 值
{
    utils_fast_sincos_better(virtual_motor.phi, (float *)&virtual_motor.sin_phi, (float *)&virtual_motor.cos_phi);

    //	Park Inverse
    virtual_motor.i_alpha = virtual_motor.cos_phi * virtual_motor.id - virtual_motor.sin_phi * virtual_motor.iq;
    virtual_motor.i_beta = virtual_motor.cos_phi * virtual_motor.iq + virtual_motor.sin_phi * virtual_motor.id;

    virtual_motor.v_alpha = virtual_motor.cos_phi * virtual_motor.vd - virtual_motor.sin_phi * virtual_motor.vq;
    virtual_motor.v_beta = virtual_motor.cos_phi * virtual_motor.vq + virtual_motor.sin_phi * virtual_motor.vd;
