 ...
 *
 */
void mcpwm_foc_set_openloop_current(float current, float rpm) { // 开环控制系列 — 用于无传感器启动阶段
 ...
 */
void mcpwm_foc_set_openloop_phase(float current, float phase) { // 开环控制系列 — 用于无传感器启动阶段
	utils_truncate_number(&current, -get_motor_now()->m_conf->l_current_max * get_motor_now()->m_conf->l_current_max_scale,
						  get_motor_now()->m_conf->l_current_max * get_motor_now()->m_conf->l_current_max_scale);
 ...
 * The RPM to use.
 */
void mcpwm_foc_set_openloop_duty(float dutyCycle, float rpm) { // 开环控制系列 — 用于无传感器启动阶段
 ...
 * The phase to use in degrees, range [0.0 360.0]
 */
void mcpwm_foc_set_openloop_duty_phase(float dutyCycle, float phase) { // 开环控制系列 — 用于无传感器启动阶段
...
float mcpwm_foc_get_phase_observer(void) { // 获取各种来源的转子位置
 ...
 * The fault code
 */
int mcpwm_foc_encoder_detect(float current, bool print, float *offset, float *ratio, bool *inverted) { // 自动检测编码器偏移
 ...
 * The fault code.
 */
int mcpwm_foc_measure_resistance(float current, int samples, bool stop_after, float *resistance) { // 自动测量电机相电阻
	mc_interface_lock();
 ...
 * The fault code
 */
int mcpwm_foc_measure_inductance(float duty, int samples, float *curr, float *ld_lq_diff, float *inductance) { // 自动测量电机电感
	volatile motor_all_state_t *motor = get_motor_now();
	int fault = FAULT_CODE_NONE;
...
bool mcpwm_foc_beep(float freq, float time, float voltage) { // 音频输出
 ...
 * The fault code
 */
int mcpwm_foc_hall_detect(float current, uint8_t *hall_table, bool *result) { // 自动检测霍尔传感器换相表
	volatile motor_all_state_t *motor = get_motor_now();
	int fault = FAULT_CODE_NONE;
	mc_interface_lock();
...
int mcpwm_foc_dc_cal(bool cal_undriven) { // ADC 直流偏置校准
	// Wait max 5 seconds for DRV-fault to go away
	int cnt = 0;
	while(IS_DRV_FAULT()){
		chThdSleepMilliseconds(1);
...
static void hfi_update(volatile motor_all_state_t *motor, float dt) { // 高频注入线程
	(void)dt;
 ...
 */
static void control_current(motor_all_state_t *motor, float dt) { // 电流环核心 — dq 轴 PI 控制、解耦、反 Park → SVM
