 ...
 */
float mc_interface_get_battery_level(float *wh_left) // 计算电池剩余电量
{
    ...
    case BATTERY_TYPE_LIIRON_2_6__3_6:
        ...
        ah_left = utils_map(v_in / (float)(conf->si_battery_cells), 2.6, 3.6, 0.0, conf->si_battery_ah);
        break;
    ...
    case BATTERY_TYPE_LEAD_ACID:
        ...
        ah_left = utils_map(v_in / (float)(conf->si_battery_cells), 2.1, 2.36, 0.0, conf->si_battery_ah);
        break;
 ...
 */
float mc_interface_get_distance(void) // 计算车速和里程
{
    const volatile mc_configuration *conf = mc_interface_get_configuration();
...
void mc_interface_mc_timer_isr(bool is_second_motor)
{
    ...
    bool sample = false;
    debug_sampling_mode sample_mode = m_sample_is_second_motor == is_second_motor ? m_sample_mode : DEBUG_SAMPLING_OFF;
...
static void update_override_limits(volatile motor_if_state_t *motor, volatile mc_configuration *conf)
{
    ...
    case TEMP_SENSOR_KTY83_122: {
        ...
        float pow2 = res * res;
        temp_motor = 0.0000000102114874947423 * pow2 * res - 0.000069967997703501 * pow2 + 0.243402040973194 * res - 160.145048329356;
    } break;

    case TEMP_SENSOR_KTY84_130: {
        float res = NTC_RES_MOTOR(ADC_Value[is_motor_1 ? ADC_IND_TEMP_MOTOR : ADC_IND_TEMP_MOTOR_2]);
        temp_motor =
            -7.82531699e-12 * res * res * res * res + 6.34445902e-8 * res * res * res - 0.00020119157 * res * res + 0.407683016 * res - 161.357536;
    } break;
    ...
    if (motor->m_temp_fet < (temp_fet_accel_start + 0.1)) {
        ...
        lo_fet_temp_accel = utils_map(motor->m_temp_fet, temp_fet_accel_start, temp_fet_accel_end, l_current_max_tmp, 0.0);
    }
    ...
    if (motor->m_temp_motor < (temp_motor_accel_start + 0.1)) {
        ...
        lo_motor_temp_accel = utils_map(motor->m_temp_motor, temp_motor_accel_start, temp_motor_accel_end, l_current_max_tmp, 0.0);
    }
    ...
    if (rpm_abs < conf->foc_start_curr_dec_rpm) {
        lo_max_curr_dec = utils_map(rpm_abs, 0, conf->foc_start_curr_dec_rpm, conf->foc_start_curr_dec * l_current_max_tmp, l_current_max_tmp);
    }
    ...
    if (duty_now_abs < (conf->l_duty_start * conf->l_max_duty) || conf->l_duty_start > 0.99) {
        ...
        lo_max_duty =
            utils_map(duty_now_abs, (conf->l_duty_start * conf->l_max_duty), conf->l_max_duty, l_current_max_tmp, conf->cc_min_current * 5.0);
    }
    ...
    if (v_in < (conf->l_battery_regen_cut_start + 0.1)) {
        ...
        lo_in_min_batt = utils_map(v_in, conf->l_battery_regen_cut_start, conf->l_battery_regen_cut_end, conf->l_in_current_min, 0.0);
    }
    ...
    if (motor->m_i_in_filter > 0.0 && conf->l_in_current_map_start < 0.98) {
        ...
        if (frac > conf->l_in_current_map_start) {
            lo_max_i_in = utils_map(frac, conf->l_in_current_map_start, 1.0, l_current_max_tmp, 0.0);
        }
...
static void send_sample_block(int ind, int offset)
{
    ...
    if (m_sample_raw) {
        ...
        buffer_append_float32_auto(buffer, ((float)m_ph1_samples[ind_samp] / 4096.0 * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                   &index);
        buffer_append_float32_auto(buffer, ((float)m_ph2_samples[ind_samp] / 4096.0 * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                   &index);
        buffer_append_float32_auto(buffer, ((float)m_ph3_samples[ind_samp] / 4096.0 * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_PH_FACTOR,
                                   &index);
        buffer_append_float32_auto(
            buffer, ((float)m_vzero_samples[ind_samp] / 4096.0 * V_REG) * ((VIN_R1 + VIN_R2) / VIN_R2) * ADC_VOLTS_INPUT_FACTOR, &index);
