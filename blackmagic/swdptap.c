...
// 读入 1 位数据。切换 SWDIO 为浮空→读引脚→送出 2 个时钟脉冲
...
// 写出 1 位数据。切换 SWDIO 为驱动→设值→输出 1 个时钟脉冲
...
// 读入 ticks 位，打包为 uint32_t 返回（LSB 优先）
