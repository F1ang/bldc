...
static bool read_eeprom_var(eeprom_var *v, int address, uint16_t base)
{
    ...
    if (EE_ReadVariable(base + 2 * address, &var0) == 0 && EE_ReadVariable(base + 2 * address + 1, &var1) == 0) {
...
bool conf_general_detect_motor_param(float current, float min_rpm, float low_duty, float *int_limit, float *bemf_coupling_k, int8_t *hall_table,
                                     int *hall_res)
{
 ...
 */
bool conf_general_measure_flux_linkage(float current, float duty, float min_erpm, float res, float *linkage)
{
 ...
 * Fault code
 */
int conf_general_measure_flux_linkage_openloop(float current, float duty, float erpm_per_sec, float res, float ind, float *linkage,
...
static int measure_r_l_imax(float current_min, float current_max, float max_power_loss, float *r, float *l, float *ld_lq_diff, float *i_max)
{
...
static void measure_r_l_imax_task(void *arg)
{
    ...
    args->fault = measure_r_l_imax(args->current_min, args->current_max, args->max_power_loss, &args->r, &args->l, &args->ld_lq_diff, &args->i_max);
}
...
static void measure_flux_linkage_task(void *arg)
{
    ...
    args->fault = conf_general_measure_flux_linkage_openloop(args->current, args->duty, args->erpm_per_sec, args->res, args->ind, &linkage,
                                                             &linkage_undriven, &undriven_samples, &args->result);
...
static void detect_sensors_task(void *arg)
{
    ...
    args->fault = conf_general_autodetect_apply_sensors_foc(args->current, args->store_mcconf_on_success, args->send_mcconf_on_success, &args->res);
}
#endif
...
int conf_general_detect_apply_all_foc(float max_power_loss, bool store_mcconf_on_success, bool send_mcconf_on_success)
{
    ...
    *mcconf_old_second = *mcconf_second;
    ...
    float ld_lq_diff;
    float i_max = 0.0;
    faultM1 = measure_r_l_imax(mcconf->cc_min_current, mcconf->l_current_max, max_power_loss, &r, &l, &ld_lq_diff, &i_max);
    ...
    bool res;
    faultM1 = conf_general_measure_flux_linkage_openloop(i_max / 2.5, 0.3, 1800, r, l, &lambda, &lambda_undriven, &lambda_undriven_samples, &res);
    ...
    if (res && res_linkage_m2) {
        ...
        faultM1 = conf_general_autodetect_apply_sensors_foc(i_max / 3.0, store_mcconf_on_success, send_mcconf_on_success, &result);
 ...
 */
int conf_general_detect_apply_all_foc_can(bool detect_can, float max_power_loss, float min_current_in, float max_current_in, float openloop_rpm,
                                          float sl_erpm, void (*reply_func)(unsigned char *data, unsigned int len))
{
