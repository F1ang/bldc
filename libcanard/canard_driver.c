...
static const uint32_t flash_addr[FLASH_SECTORS] = {ADDR_FLASH_SECTOR_0, ADDR_FLASH_SECTOR_1, ADDR_FLASH_SECTOR_2,  ADDR_FLASH_SECTOR_3,
 ...
 */
static param_t parameters[] = {{"can_baud_rate", AP_PARAM_INT8, 0, 0, 8, CAN_BAUD_500K}, {"can_status_rate_1", AP_PARAM_INT32, 0, 0, 1000, 50},
                               {"can_status_rate_2", AP_PARAM_INT32, 0, 0, 1000, 5},     {"can_status_msgs_r1", AP_PARAM_INT16, 0, 0, 255, 0},
                               {"can_status_msgs_r2", AP_PARAM_INT16, 0, 0, 255, 0},     {"can_esc_index", AP_PARAM_INT16, 0, 0, 255, 0},
                               {"controller_id", AP_PARAM_INT16, 0, 0, 253, 0},          {"ctl_dir", AP_PARAM_INT8, 0, 0, 1, 0}};
...
void canard_driver_init(void)
{
    ...
    terminal_register_command_callback("uavcan_debug", "Enable UAVCAN debug prints 0: off 1: errors 2: param getset 3: current calc 4: comms stuff)",
                                       "[level]", terminal_debug_on);
}
...
static void sendNodeStatus(CanardInstance *ins)
{
    ...
    canardBroadcast(ins, UAVCAN_PROTOCOL_NODESTATUS_SIGNATURE, UAVCAN_PROTOCOL_NODESTATUS_ID, &transfer_id, CANARD_TRANSFER_PRIORITY_LOW, msg_buffer,
...
static void sendEscStatus(CanardInstance *ins)
{
    ...
    status.power_rating_pct = (fabsf(mc_interface_get_tot_current()) / conf->l_current_max * conf->l_current_max_scale) * 100.0;
    ...
    canardBroadcast(ins, UAVCAN_EQUIPMENT_ESC_STATUS_SIGNATURE, UAVCAN_EQUIPMENT_ESC_STATUS_ID, &transfer_id, CANARD_TRANSFER_PRIORITY_LOW,
                    msg_buffer, UAVCAN_EQUIPMENT_ESC_STATUS_MAX_SIZE);
}
...
static void sendRtData(CanardInstance *ins)
{
    ...
    canardBroadcast(ins, VESC_RTDATA_SIGNATURE, VESC_RTDATA_ID, &transfer_id, CANARD_TRANSFER_PRIORITY_LOW, msg_buffer, VESC_RTDATA_MAX_SIZE);
}
...
static void handle_get_node_info(CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    pkt.software_version.optional_field_flags =
        UAVCAN_PROTOCOL_SOFTWAREVERSION_OPTIONAL_FIELD_FLAG_VCS_COMMIT | UAVCAN_PROTOCOL_SOFTWAREVERSION_OPTIONAL_FIELD_FLAG_IMAGE_CRC;
    ...
    const int16_t resp_res =
        canardRequestOrRespond(ins, transfer->source_node_id, UAVCAN_PROTOCOL_GETNODEINFO_SIGNATURE, UAVCAN_PROTOCOL_GETNODEINFO_ID,
                               &transfer->transfer_id, transfer->priority, CanardResponse, msg_buffer, total_size);
    if (resp_res <= 0) {
        if (debug_level > 1) {
...
static void handle_param_getset(CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    const int16_t resp_res =
        canardRequestOrRespond(ins, transfer->source_node_id, UAVCAN_PROTOCOL_PARAM_GETSET_SIGNATURE, UAVCAN_PROTOCOL_PARAM_GETSET_ID,
                               &transfer->transfer_id, transfer->priority, CanardResponse, &msg_buffer[0], total_size);
...
static void send_fw_read(CanardInstance *ins)
{
    ...
    canardRequestOrRespond(ins, fw_update.node_id, UAVCAN_PROTOCOL_FILE_READ_SIGNATURE, UAVCAN_PROTOCOL_FILE_READ_ID, &fw_update.transfer_id,
                           CANARD_TRANSFER_PRIORITY_HIGH, CanardRequest, &msg_buffer[0], total_size);
}
...
static void handle_file_read_response(CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    if ((transfer->transfer_id + 1) % 256 != fw_update.transfer_id || transfer->source_node_id != fw_update.node_id) {
        return;
    }
    int16_t error = 0;
...
static void handle_begin_firmware_update(CanardInstance *ins, CanardRxTransfer *transfer)
{
    ...
    canardRequestOrRespond(ins, transfer->source_node_id, UAVCAN_PROTOCOL_FILE_BEGINFIRMWAREUPDATE_SIGNATURE,
                           UAVCAN_PROTOCOL_FILE_BEGINFIRMWAREUPDATE_ID, &transfer->transfer_id, transfer->priority, CanardResponse, &msg_buffer[0],
                           total_size);
...
{
    if (debug_level == 4) {
        commands_printf("UAVCAN transfer RX: NODE: %d Type: %d ID: %d", transfer->source_node_id, transfer->transfer_type, transfer->data_type_id);
    }
                                 ...
                                 CanardTransferType transfer_type, uint8_t source_node_id)
{
    ...
    if (debug_level == 4) {
        commands_printf("UAVCAN shouldAccept: NODE: %d Type: %d ID: %d", source_node_id, transfer_type, data_type_id);
    }
...
/*
    1.检查 CAN_MODE_UAVCAN 模式
    2.延时初始化 CanardInstance
    3.设置本地节点 ID
    4.从 CAN 接收帧 → canardHandleRxFrame() 处理
    5.TX 队列帧 → comm_can_transmit_eid_if() 发送
    6.每秒：清理过期传输、发送节点状态
    7.按配置频率发送电调状态和实时数据
    8.按 CURRENT_CALC_FREQ_HZ（10Hz）计算总电流
    9.按 PARAM_REFRESH_RATE_HZ（10Hz）同步参数
    10.固件下载中持续发送文件读取请求
    11.固件下载完成后延迟 500ms 跳转引导加载器
*/
static THD_FUNCTION(canard_thread, arg)
{
    (void)arg;
    ...
    for (;;) {
        ...
        if (!was_running) {
            ...
            canardInit(&canard_ins, canard_memory_pool, sizeof(canard_memory_pool), onTransferReceived, shouldAcceptTransfer, NULL);

#ifdef HW_CAN2_DEV
            memset(&canard_ins_if2, 0, sizeof(canard_ins_if2));
            memset(canard_memory_pool_if2, 0, sizeof(canard_memory_pool_if2));

            canardInit(&canard_ins_if2, canard_memory_pool_if2, sizeof(canard_memory_pool_if2), onTransferReceived, shouldAcceptTransfer, NULL);
#endif
