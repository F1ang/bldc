 ...
 */
uint32_t uavcan_protocol_NodeStatus_encode_internal(uavcan_protocol_NodeStatus *source, void *msg_buf, uint32_t offset,
                                                    uint8_t CANARD_MAYBE_UNUSED(root_item))
{
    canardEncodeScalar(msg_buf, offset, 32, (void *)&source->uptime_sec); // 4294967295
    offset += 32;

    source->health = CANARD_INTERNAL_SATURATE_UNSIGNED(source->health, 3) canardEncodeScalar(msg_buf, offset, 2, (void *)&source->health); // 3
    offset += 2;

    source->mode = CANARD_INTERNAL_SATURATE_UNSIGNED(source->mode, 7) canardEncodeScalar(msg_buf, offset, 3, (void *)&source->mode); // 7
    offset += 3;

    source->sub_mode = CANARD_INTERNAL_SATURATE_UNSIGNED(source->sub_mode, 7) canardEncodeScalar(msg_buf, offset, 3, (void *)&source->sub_mode); // 7
    offset += 3;
 ...
 */
int32_t uavcan_protocol_NodeStatus_decode_internal(const CanardRxTransfer *transfer, uint16_t CANARD_MAYBE_UNUSED(payload_len),
                                                   uavcan_protocol_NodeStatus *dest, uint8_t **CANARD_MAYBE_UNUSED(dyn_arr_buf), int32_t offset)
{
    int32_t ret = 0;
 ...
 * @retval offset or ERROR value if < 0
 */
int32_t uavcan_protocol_NodeStatus_decode(const CanardRxTransfer *transfer, uint16_t payload_len, uavcan_protocol_NodeStatus *dest,
                                          uint8_t **dyn_arr_buf)
{
