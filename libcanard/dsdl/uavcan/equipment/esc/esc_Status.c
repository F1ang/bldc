...
// 电调状态：错误计数、电压（f16）、电流（f16）、温度（f16）、转速（18位）、功率百分比（7位）、ESC索引（5位）
 ...
 */
uint32_t uavcan_equipment_esc_Status_encode_internal(uavcan_equipment_esc_Status *source, void *msg_buf, uint32_t offset,
                                                     uint8_t CANARD_MAYBE_UNUSED(root_item))
{
    ...
    canardEncodeScalar(msg_buf, offset, 16, (void *)&tmp_float); // 32767
    offset += 16;
    source->rpm = CANARD_INTERNAL_SATURATE(source->rpm, 131071) canardEncodeScalar(msg_buf, offset, 18, (void *)&source->rpm); // 131071
    offset += 18;
    ...
    source->esc_index =
        CANARD_INTERNAL_SATURATE_UNSIGNED(source->esc_index, 31) canardEncodeScalar(msg_buf, offset, 5, (void *)&source->esc_index); // 31
    offset += 5;
 ...
 */
int32_t uavcan_equipment_esc_Status_decode_internal(const CanardRxTransfer *transfer, uint16_t CANARD_MAYBE_UNUSED(payload_len),
                                                    uavcan_equipment_esc_Status *dest, uint8_t **CANARD_MAYBE_UNUSED(dyn_arr_buf), int32_t offset)
{
    int32_t ret = 0;
#ifndef CANARD_USE_FLOAT16_CAST
    uint16_t tmp_float = 0;
 ...
 * @retval offset or ERROR value if < 0
 */
int32_t uavcan_equipment_esc_Status_decode(const CanardRxTransfer *transfer, uint16_t payload_len, uavcan_equipment_esc_Status *dest,
                                           uint8_t **dyn_arr_buf)
{
