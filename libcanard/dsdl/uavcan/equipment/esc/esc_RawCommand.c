 ...
 */
uint32_t uavcan_equipment_esc_RawCommand_encode_internal(uavcan_equipment_esc_RawCommand *source, void *msg_buf, uint32_t offset,
                                                         uint8_t CANARD_MAYBE_UNUSED(root_item))
{
    ...
    for (c = 0; c < source->cmd.len; c++) {
        canardEncodeScalar(msg_buf, offset, 14,
 ...
 */
int32_t uavcan_equipment_esc_RawCommand_decode_internal(const CanardRxTransfer *transfer, uint16_t CANARD_MAYBE_UNUSED(payload_len),
                                                        uavcan_equipment_esc_RawCommand *dest, uint8_t **CANARD_MAYBE_UNUSED(dyn_arr_buf),
                                                        int32_t offset)
{
    int32_t ret = 0;
    uint32_t c = 0;
    ...
    for (c = 0; c < dest->cmd.len; c++) {
        if (dyn_arr_buf) {
            ret = canardDecodeScalar(transfer, (uint32_t)offset, 14, true,
 ...
 * @retval offset or ERROR value if < 0
 */
int32_t uavcan_equipment_esc_RawCommand_decode(const CanardRxTransfer *transfer, uint16_t payload_len, uavcan_equipment_esc_RawCommand *dest,
                                               uint8_t **dyn_arr_buf)
{
