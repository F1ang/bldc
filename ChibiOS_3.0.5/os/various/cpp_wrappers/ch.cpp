...
// 将 C API 封装为 C++ 类：Thread、Mutex、Semaphore、Event、Mailbox、Queue
// 利用 RAII（资源获取即初始化）管理同步对象生命周期
// 提供 ChibiOS::BaseThread::Sleep() 等更现代的方法调用风格
...
void ThreadReference::requestTerminate(void)
{
    ...
    chDbgAssert(thread_ref != NULL, "not referenced");
...
eventmask_t BaseThread::waitOneEventTimeout(eventmask_t ewmask, systime_t time)
{

    return chEvtWaitOneTimeout(ewmask, time);
}

eventmask_t BaseThread::waitAnyEventTimeout(eventmask_t ewmask, systime_t time)
{

    return chEvtWaitAnyTimeout(ewmask, time);
}

eventmask_t BaseThread::waitAllEventsTimeout(eventmask_t ewmask, systime_t time)
{
...
void BaseThread::dispatchEvents(const evhandler_t handlers[], eventmask_t mask)
{
...
msg_t CounterSemaphore::signalWait(CounterSemaphore *ssem, CounterSemaphore *wsem)
{
