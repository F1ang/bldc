...
// 注册并绑核串口终端命令
// VESC 用它实现 UART/蓝牙命令行控制，如读取电压、转速、配置参数
// 通过 shellcfg_t 配置命令表和终端参数
 ...
 */
static ShellCommand local_commands[] = {{"info", cmd_info}, {"systime", cmd_systime}, {NULL, NULL}};

static bool cmdexec(const ShellCommand *scp, BaseSequentialStream *chp, char *name, int argc, char *argv[])
{
