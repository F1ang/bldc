...
#define UBOX_POWER_EN_ON()    \
    do {                      \
        palSetPad(GPIOA, 15); \
    } while (0)
#define UBOX_POWER_EN_OFF()     \
    do {                        \
        palClearPad(GPIOA, 15); \
    } while (0)

#define UBOX_POWER_KEY_IO_PULL_LOW() \
    do {                             \
        palClearPad(GPIOC, 13);      \
    } while (0)
#define UBOX_POWER_KEY_IO_RELEASE() \
    do {                            \
        palSetPad(GPIOC, 13);       \
    } while (0)
...
void hw_start_i2c(void)
{
    ...
    if (!i2c_running) {
        palSetPadMode(HW_I2C_SCL_PORT, HW_I2C_SCL_PIN,
                      PAL_MODE_ALTERNATE(HW_I2C_GPIO_AF) | PAL_STM32_OTYPE_OPENDRAIN | PAL_STM32_OSPEED_MID1 | PAL_STM32_PUDR_PULLUP);
