...
void mpu9150_init(stm32_gpio_t *sda_gpio, int sda_pin, stm32_gpio_t *scl_gpio, int scl_pin, stkalign_t *work_area, size_t work_area_size)
{
    ...
    terminal_register_command_callback("mpu_status", "Print status of the MPU 9150/9250", 0, terminal_status);

    terminal_register_command_callback("mpu_read_reg", "Read register of the MPU 9150/9250", "[reg]", terminal_read_reg);
...
static void terminal_status(int argc, const char **argv)
{
    ...
    if (mpu_found) {
        commands_printf("Type       : %s\n"
                        "Errors     : %i\n"
                        "Errors Mag : %i\n",
                        mpu9150_is_mpu9250() ? "MPU9250" : "MPU9150", mpu9150_get_failed_reads(), mpu9150_get_failed_mag_reads());
    } else {
...
{
    return (systime_t)((float)chVTTimeElapsedSinceX(last_update_time) / ((float)CH_CFG_ST_FREQUENCY / 1000.0));
}
...
static int get_raw_accel_gyro(int16_t *accel_gyro)
{
    ...
    // Acceleration
    for (int i = 0; i < 3; i++) {
        accel_gyro[i] = ((int16_t)((uint16_t)rx_buf[2 * i] << 8) + rx_buf[2 * i + 1]);
    }

    // Angular rate
    for (int i = 4; i < 7; i++) {
        accel_gyro[i - 1] = ((int16_t)((uint16_t)rx_buf[2 * i] << 8) + rx_buf[2 * i + 1]);
    }
