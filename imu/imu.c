...
/* --------------------- 统一 IMU 接口层，管理传感器初始化、数据回调和姿态融合调度 -------------------- */
...
{
    bool imu_changed = set->sample_rate_hz != m_settings.sample_rate_hz || set->type != m_settings.type;
    ...
    if (set->type == IMU_TYPE_INTERNAL) {
        ...
        imu_init_mpu9x50(MPU9X50_SDA_GPIO, MPU9X50_SDA_PIN, MPU9X50_SCL_GPIO, MPU9X50_SCL_PIN);
        m_imu_type_internal = "MPU9X50";
#endif

#ifdef ICM20948_SDA_GPIO
        imu_init_icm20948(ICM20948_SDA_GPIO, ICM20948_SDA_PIN, ICM20948_SCL_GPIO, ICM20948_SCL_PIN, ICM20948_AD0_VAL);
        m_imu_type_internal = "ICM20948";
#endif

#ifdef BMI160_SDA_GPIO
        imu_init_bmi160_i2c(BMI160_SDA_GPIO, BMI160_SDA_PIN, BMI160_SCL_GPIO, BMI160_SCL_PIN);
        m_imu_type_internal = "BMI160";
#endif

#ifdef LSM6DS3_SDA_GPIO
        imu_init_lsm6ds3(LSM6DS3_SDA_GPIO, LSM6DS3_SDA_PIN, LSM6DS3_SCL_GPIO, LSM6DS3_SCL_PIN);
        ...
        imu_init_lsm6ds3(LSM6DS3_MOSI_GPIO, LSM6DS3_MOSI_PIN, LSM6DS3_SCK_GPIO, LSM6DS3_SCK_PIN);
        m_imu_type_internal = "LSM6DS3";
#endif

#ifdef BMI160_SPI_PORT_NSS
        imu_init_bmi160_spi(BMI160_SPI_PORT_NSS, BMI160_SPI_PIN_NSS, BMI160_SPI_PORT_SCK, BMI160_SPI_PIN_SCK, BMI160_SPI_PORT_MOSI,
                            BMI160_SPI_PIN_MOSI, BMI160_SPI_PORT_MISO, BMI160_SPI_PIN_MISO);
        m_imu_type_internal = "BMI160_SPI";
#endif
    } else if (set->type == IMU_TYPE_EXTERNAL_MPU9X50) {
        imu_init_mpu9x50(HW_I2C_SDA_PORT, HW_I2C_SDA_PIN, HW_I2C_SCL_PORT, HW_I2C_SCL_PIN);
    } else if (set->type == IMU_TYPE_EXTERNAL_ICM20948) {
        imu_init_icm20948(HW_I2C_SDA_PORT, HW_I2C_SDA_PIN, HW_I2C_SCL_PORT, HW_I2C_SCL_PIN, 0);
    } else if (set->type == IMU_TYPE_EXTERNAL_BMI160) {
        imu_init_bmi160_i2c(HW_I2C_SDA_PORT, HW_I2C_SDA_PIN, HW_I2C_SCL_PORT, HW_I2C_SCL_PIN);
    } else if (set->type == IMU_TYPE_EXTERNAL_LSM6DS3) {
        imu_init_lsm6ds3(HW_I2C_SDA_PORT, HW_I2C_SDA_PIN, HW_I2C_SCL_PORT, HW_I2C_SCL_PIN);
    } else if (set->type == IMU_TYPE_EXTERNAL_BMI160) {
        imu_init_bmi160_i2c(HW_I2C_SDA_PORT, HW_I2C_SDA_PIN, HW_I2C_SCL_PORT, HW_I2C_SCL_PIN);
    }

    terminal_register_command_callback("imu_type_internal", "Print internal IMU type", 0, terminal_imu_type_internal);
}
...
void imu_init_mpu9x50(stm32_gpio_t *sda_gpio, int sda_pin, stm32_gpio_t *scl_gpio, int scl_pin)
{
    ...
    mpu9150_init(sda_gpio, sda_pin, scl_gpio, scl_pin, m_thd_work_area, sizeof(m_thd_work_area));
    mpu9150_set_read_callback(imu_read_callback);
}

void imu_init_icm20948(stm32_gpio_t *sda_gpio, int sda_pin, stm32_gpio_t *scl_gpio, int scl_pin, int ad0_val)
{
    imu_stop();
    ...
    icm20948_init(&m_icm20948_state, &m_i2c_bb, ad0_val, m_thd_work_area, sizeof(m_thd_work_area));
...
void imu_init_bmi160_spi(stm32_gpio_t *nss_gpio, int nss_pin, stm32_gpio_t *sck_gpio, int sck_pin, stm32_gpio_t *mosi_gpio, int mosi_pin,
...
// 1. 保存设置 → Madgwick 模式 + 1000Hz
// 2. 重置姿态 → 等待 1.5s
// 3. 采样 250ms 滚动 → 取平均 → 设置 rot_roll 归零
// 4. 旋转陀螺偏移量
// 5. 重置姿态 → 等待 1.5s
// 6. 采样 250ms 俯仰 → 取平均 → 设置 rot_pitch 归零
// 7. 旋转陀螺偏移量
// 8. 设置 rot_yaw = 用户指定角度
// 9. 最终旋转陀螺偏移量
// 10. 输出 9 个校准值到 imu_cal[]
// 11. 恢复原始设置
void imu_get_calibration(float yaw, float *imu_cal)
{
    ...
    ahrs_update_all_parameters(&m_att, m_settings.accel_confidence_decay, m_settings.mahony_kp, m_settings.mahony_ki, m_settings.madgwick_beta);
