...
/* -------------------- BMI160 六轴 IMU 驱动封装（支持 I2C 和 SPI） -------------------- */
/* -------------- 6 轴 IMU，无内置磁力计同时支持 I2C 和 SPI 使用 BMI160 官方驱动库 -------------- */
// Threads
...
static THD_FUNCTION(bmi_thread, arg)
{
    ...
    for (;;) {
        ...
        int8_t res = bmi160_get_sensor_data((BMI160_ACCEL_SEL | BMI160_GYRO_SEL), &accel, &gyro, &(s->sensor));
