...
/* ---- 主命令调度器，75KB 代码量，解析 VESC Tool 发来的全部命令包（读取电压、电流、转速、设置 PID 参数、更新固件等） --- */
...
void commands_process_packet(unsigned char *data, unsigned int len, void (*reply_func)(unsigned char *data, unsigned int len))
{
    ...
    case COMM_SET_MCCONF_TEMP_SETUP: {
        ...
        *mcconf = *mc_interface_get_configuration();
        ...
        if (packet_id == COMM_SET_MCCONF_TEMP_SETUP) {
            const float fact = ((mcconf->si_motor_poles / 2.0) * 60.0 * mcconf->si_gear_ratio) / (mcconf->si_wheel_diameter * M_PI);
    ...
    case COMM_LISP_WRITE_CODE: {
        ...
        uint16_t flash_res =
            flash_helper_write_code(packet_id == COMM_QMLUI_WRITE ? CODE_IND_QML : CODE_IND_LISP, qmlui_offset, data + ind, len - ind);
...
static THD_FUNCTION(blocking_thread, arg)
{
    ...
    for (;;) {
        ...
        case COMM_DETECT_MOTOR_PARAM: {
            ...
            if (!conf_general_detect_motor_param(detect_current, detect_min_rpm, detect_low_duty, &detect_cycle_int_limit, &detect_coupling_k,
        ...
        case COMM_DETECT_MOTOR_FLUX_LINKAGE_OPENLOOP: {
            ...
            bool res;
            int fault = conf_general_measure_flux_linkage_openloop(current, duty, erpm_per_sec, resistance, inductance, &linkage, &linkage_undriven,
        ...
        case COMM_DETECT_APPLY_ALL_FOC: {
            ...
            int res = conf_general_detect_apply_all_foc_can(detect_can, max_power_loss, min_current_in, max_current_in, openloop_rpm, sl_erpm,
                                                            send_func_blocking);
