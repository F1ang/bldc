...
/* --------------- 数据包协议层，负责封包/解包（SOF + 长度 + 类型 + 数据 + CRC32） --------------- */

// Private functions
static int try_decode_packet(unsigned char *buffer, unsigned int in_len, void (*process_func)(unsigned char *data, unsigned int len),
                             int *bytes_left);

void packet_init(void (*s_func)(unsigned char *data, unsigned int len), void (*p_func)(unsigned char *data, unsigned int len), PACKET_STATE_t *state)
{
...
void packet_process_byte(uint8_t rx_data, PACKET_STATE_t *state)
{
    ...
    for (;;) {
        int res = try_decode_packet(state->rx_buffer + state->rx_read_ptr, data_len, state->process_func, &state->bytes_left);
 ...
 */
static int try_decode_packet(unsigned char *buffer, unsigned int in_len, void (*process_func)(unsigned char *data, unsigned int len), int *bytes_left)
{
    *bytes_left = 0;
...
#define is_len_24b false
    ...
    if (is_len_8b) {
        ...
        len = (unsigned int)buffer[1] << 16 | (unsigned int)buffer[2] << 8 | (unsigned int)buffer[3];
    ...
    unsigned short crc_rx = (unsigned short)buffer[data_start + len] << 8 | (unsigned short)buffer[data_start + len + 1];
