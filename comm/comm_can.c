...
void comm_can_init(void)
{
    ...
    palSetPadMode(HW_CANRX_PORT, HW_CANRX_PIN, PAL_MODE_ALTERNATE(HW_CAN_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_OSPEED_MID1);
    palSetPadMode(HW_CANTX_PORT, HW_CANTX_PIN, PAL_MODE_ALTERNATE(HW_CAN_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_OSPEED_MID1);

#ifdef HW_CAN2_DEV
    memset(&m_rx_state2, 0, sizeof(m_rx_state2));

    palSetPadMode(HW_CAN2_RX_PORT, HW_CAN2_RX_PIN, PAL_MODE_ALTERNATE(HW_CAN2_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_OSPEED_MID1);
    palSetPadMode(HW_CAN2_TX_PORT, HW_CAN2_TX_PIN, PAL_MODE_ALTERNATE(HW_CAN2_GPIO_AF) | PAL_STM32_OTYPE_PUSHPULL | PAL_STM32_OSPEED_MID1);
    ...
    chThdCreateStatic(cancom_status_thread_2_wa, sizeof(cancom_status_thread_2_wa), NORMALPRIO, cancom_status_thread_2, NULL);
    chThdCreateStatic(cancom_process_thread_wa, sizeof(cancom_process_thread_wa), NORMALPRIO, cancom_process_thread, NULL);
#ifdef HW_HAS_DUAL_MOTORS
    chThdCreateStatic(cancom_status_internal_thread_wa, sizeof(cancom_status_internal_thread_wa), NORMALPRIO, cancom_status_internal_thread, NULL);
#endif
...
CAN_BAUD comm_can_kbits_to_baud(int kbits)
{
    ...
    switch (kbits) {
    case 125:
        new_baud = CAN_BAUD_125K;
        break;
    case 250:
        new_baud = CAN_BAUD_250K;
        break;
    case 500:
        new_baud = CAN_BAUD_500K;
        break;
    case 1000:
        new_baud = CAN_BAUD_1M;
        break;
    case 10:
        new_baud = CAN_BAUD_10K;
        break;
    case 20:
        new_baud = CAN_BAUD_20K;
        break;
    case 50:
        new_baud = CAN_BAUD_50K;
        break;
    case 75:
        new_baud = CAN_BAUD_75K;
        break;
    case 100:
        new_baud = CAN_BAUD_100K;
        break;
    default:
        new_baud = CAN_BAUD_INVALID;
        break;
    }
...
msg_t comm_can_transmit_eid_replace(uint32_t id, const uint8_t *data, uint8_t len, bool replace, int interface)
{
    ...
    if (app_get_configuration()->can_mode == CAN_MODE_VESC) {
        if (replace && ((id & 0xFF) == utils_second_motor_id() || (id & 0xFF) == app_get_configuration()->controller_id)) {
            uint8_t data_tmp[10];
...
void comm_can_send_buffer(uint8_t controller_id, uint8_t *data, unsigned int len, uint8_t send)
{
    ...
    if (len <= 6) {
        ...
        ind += len;
        comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PROCESS_SHORT_BUFFER << 8), send_buffer, ind, true, 0);
    } else {
        ...
        for (unsigned int i = 0; i < len; i += 7) {
            ...
            comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_FILL_RX_BUFFER << 8), send_buffer, send_len + 1, true, 0);
        }
        ...
        for (unsigned int i = end_a; i < len; i += 6) {
            ...
            comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_FILL_RX_BUFFER_LONG << 8), send_buffer, send_len + 2, true, 0);
        }
        ...
        comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PROCESS_RX_BUFFER << 8), send_buffer, ind++, true, 0);
    }
}
...
void comm_can_set_duty(uint8_t controller_id, float duty)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_DUTY << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_current(uint8_t controller_id, float current)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_current_brake(uint8_t controller_id, float current)
{
    ...
    uint8_t buffer[4];
    buffer_append_int32(buffer, (int32_t)(current * 1000.0), &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_rpm(uint8_t controller_id, float rpm)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_RPM << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_pos(uint8_t controller_id, float pos)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_POS << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_current_rel(uint8_t controller_id, float current_rel)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_REL << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_current_brake_rel(uint8_t controller_id, float current_rel)
{
    ...
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_BRAKE_REL << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_handbrake(uint8_t controller_id, float current)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE << 8), buffer, send_index, true, 0);
}
...
void comm_can_set_handbrake_rel(uint8_t controller_id, float current_rel)
{
    ...
    uint8_t buffer[4];
    buffer_append_float32(buffer, current_rel, 1e5, &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_SET_CURRENT_HANDBRAKE_REL << 8), buffer, send_index, true, 0);
}
...
bool comm_can_ping(uint8_t controller_id, HW_TYPE *hw_type)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_PING << 8), buffer, 1, true, 0);
...
void comm_can_detect_apply_all_foc(uint8_t controller_id, bool activate_status_msgs, float max_power_loss)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC << 8), buffer, send_index, true, 0);
}
...
void comm_can_conf_current_limits(uint8_t controller_id, bool store, float min, float max)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)(store ? CAN_PACKET_CONF_STORE_CURRENT_LIMITS : CAN_PACKET_CONF_CURRENT_LIMITS) << 8),
 ...
 */
void comm_can_conf_foc_erpms(uint8_t controller_id, bool store, float foc_openloop_rpm, float foc_sl_erpm)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    buffer_append_float32(buffer, foc_openloop_rpm, 1e3, &send_index);
    buffer_append_float32(buffer, foc_sl_erpm, 1e3, &send_index);
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)(store ? CAN_PACKET_CONF_STORE_FOC_ERPMS : CAN_PACKET_CONF_FOC_ERPMS) << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_conf_battery_cut(uint8_t controller_id, bool store, float start, float end)
{
    ...
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)(store ? CAN_PACKET_CONF_STORE_BATTERY_CUT : CAN_PACKET_CONF_BATTERY_CUT) << 8), buffer,
                                  send_index, true, 0);
}
...
void comm_can_shutdown(uint8_t controller_id)
{
    int32_t send_index = 0;
    uint8_t buffer[8];
    comm_can_transmit_eid_replace(controller_id | ((uint32_t)(CAN_PACKET_SHUTDOWN) << 8), buffer, send_index, true, 0);
}
...
void comm_can_send_update_baud(int kbits, int delay_msec)
{
    ...
    comm_can_transmit_eid_replace(255 | ((uint32_t)CAN_PACKET_UPDATE_BAUD << 8), buffer, send_index, false, 0);
}
...
void comm_can_io_board_set_output_digital(int id, int channel, bool on)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_DIGITAL << 8), buffer, send_index, true, 0);
}
...
void comm_can_io_board_set_output_pwm(int id, int channel, float duty)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_IO_BOARD_SET_OUTPUT_PWM << 8), buffer, send_index, true, 0);
}
...
void comm_can_psw_switch(int id, bool is_on, bool plot)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_PSW_SWITCH << 8), buffer, send_index, true, 0);
}
...
void comm_can_update_pid_pos_offset(int id, float angle_now, bool store)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_UPDATE_PID_POS_OFFSET << 8), buffer, send_index, true, 0);
}
...
void comm_can_send_status1(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS << 8), buffer, send_index, replace, 0);
}
...
void comm_can_send_status2(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_2 << 8), buffer, send_index, replace, 0);
}
...
void comm_can_send_status3(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_3 << 8), buffer, send_index, replace, 0);
}
...
void comm_can_send_status4(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_4 << 8), buffer, send_index, replace, 0);
}
...
void comm_can_send_status5(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_5 << 8), buffer, send_index, replace, 0);
}
...
void comm_can_send_status6(uint8_t id, bool replace)
{
    ...
    comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS_6 << 8), buffer, send_index, replace, 0);
}
...
static THD_FUNCTION(cancom_process_thread, arg)
{
    ...
    for (;;) {
        ...
        if (app_get_configuration()->can_mode == CAN_MODE_UAVCAN) {
            continue;
        } else if (app_get_configuration()->can_mode == CAN_MODE_COMM_BRIDGE || app_get_configuration()->can_mode == CAN_MODE_UNUSED) {
            ...
            while ((rxmsg_tmp = comm_can_get_rx_frame(0)) != 0) {
                ...
                if (app_get_configuration()->can_mode == CAN_MODE_COMM_BRIDGE) {
                    commands_fwd_can_frame(rxmsg.DLC, rxmsg.data8, rxmsg.IDE == CAN_IDE_EXT ? rxmsg.EID : rxmsg.SID, rxmsg.IDE == CAN_IDE_EXT);
                }
    ...
    if (id == 255 || id == id1 || id == id2) {
        ...
        case CAN_PACKET_PROCESS_RX_BUFFER: {
            ...
            if (crc16(rx_buffer[buf_ind], rxbuf_len) == ((unsigned short)crc_high << 8 | (unsigned short)crc_low)) {
                ...
                if (is_replaced) {
                    if (rx_buffer[buf_ind][0] == COMM_JUMP_TO_BOOTLOADER || rx_buffer[buf_ind][0] == COMM_ERASE_NEW_APP ||
                        rx_buffer[buf_ind][0] == COMM_WRITE_NEW_APP_DATA || rx_buffer[buf_ind][0] == COMM_WRITE_NEW_APP_DATA_LZO ||
        ...
        case CAN_PACKET_PROCESS_SHORT_BUFFER: {
            ...
            if (is_replaced) {
                if (data8[ind] == COMM_JUMP_TO_BOOTLOADER || data8[ind] == COMM_ERASE_NEW_APP || data8[ind] == COMM_WRITE_NEW_APP_DATA ||
                    data8[ind] == COMM_WRITE_NEW_APP_DATA_LZO || data8[ind] == COMM_ERASE_BOOTLOADER) {
                    break;
                }
            }
        ...
        case CAN_PACKET_PING: {
            ...
            comm_can_transmit_eid_replace(data8[0] | ((uint32_t)CAN_PACKET_PONG << 8), buffer, 2, true, 0);
        } break;
        ...
        case CAN_PACKET_DETECT_APPLY_ALL_FOC: {
            ...
            int8_t buffer[1];
            buffer[0] = res;
            comm_can_transmit_eid_replace(data8[0] | ((uint32_t)CAN_PACKET_DETECT_APPLY_ALL_FOC_RES << 8), (uint8_t *)buffer, 1, true, 0);
        } break;
        ...
        case CAN_PACKET_POLL_TS5700N8501_STATUS: {
            comm_can_transmit_eid_replace(app_get_configuration()->controller_id | ((uint32_t)CAN_PACKET_POLL_TS5700N8501_STATUS << 8),
        ...
        case CAN_PACKET_POLL_ROTOR_POS: {
            ...
            comm_can_transmit_eid_replace(app_get_configuration()->controller_id | ((uint32_t)CAN_PACKET_POLL_ROTOR_POS << 8), (uint8_t *)buffer, 4,
                                          true, 0);
        } break;
